$(document).ready(function(){
                          ...
                          <div class="card-footer">
                              <div class="text-right">`
                                  if(datos.estado =='A'){
                                      ...
                                      <button id="${datos.id}"
                                      nombre="${datos.nombre}"
                                      class="editar_marca btn btn-circle btn-sm btn-outline-success ml-2" title="Editar marca" data-toggle="modal" data-target="#editar_marca"><i class="fas fa-pen-alt"></i></button>
                                      <button id="${datos.id}"
                                      nombre="${datos.nombre}"
                                      class="borrar btn btn-circle btn-sm btn-outline-danger" title="Eliminar marca"><i class="fas fa-trash"></i></button>
                                      `;
                                  }else if(datos.estado == 'I'){
                                      template+=`
                                          <button id="${datos.id}"
                                          nombre="${datos.nombre}"
                                          funcion="activar_marca" class="activar btn btn-circle btn-sm btn-outline-dark ml-2 mr-2 mb-2" title="Activar marca"><i class="fas fa-plus"></i></button>
  ...
  async function registro_marcas(datos){
      let data = await fetch('../controllers/MarcaController.php',{
        method: "POST",
      ...
      if(data.ok){
        ...
        try {
          ...
          if(respuesta.mensaje == 'success'){
              toastr.success('Se registro el marca','Exito!')
              obtener_marcas();
              $('#crear_marca').modal('hide');
              $("#form-registrar").trigger('reset');
          }else if(respuesta.mensaje == 'error_marca'){
              Swal.fire({
                  icon: 'error',
                  title: 'El marca ya existe',
                  text: 'El marca ya existe, pongase en contacto con el administrador del sistema'
              });
              $("#form-registrar").trigger('reset');
  ...
  $.validator.setDefaults({
      submitHandler: function () {
          ...
          let funcion = "registro_marcas";
          datos.append('funcion',funcion);
          registro_marcas(datos);
      }
  });
  ...
  $(document).on('click','.editar_marca',(e)=>{
  ...
  async function editar_marcas(datos){
      let data = await fetch('../controllers/MarcaController.php',{
        method: "POST",
        //en este caso ya no necesita headers por tema de fordata
        //headers: {'Content-Type': 'application/x-www-form-urlencoded'},
      ...
      if(data.ok){
        ...
        try {
          let respuesta = JSON.parse(response);
          if(respuesta.mensaje == 'success'){
              toastr.success('Se Edito el marca','Exito!')
              obtener_marcas();
  ...
  $(document).on('click','.borrar',(e)=>{
      ...
      swalWithBootstrapButtons.fire({
          title: 'Deseas eliminar marca '+nombre+'?',
          icon: "warning",
          ...
          if (result.isConfirmed) {
              eliminar(id).then(respuesta=>{
                  if(respuesta.mensaje == 'success'){
                      swalWithBootstrapButtons.fire({
                          title: "Eliminado!",
                          text: "El marca fue eliminado "+nombre+".",
                          icon: "success"
                      });
                      obtener_marcas();
                  }else if(respuesta.mensaje == 'error_sesion'){
            ...
            /* Read more about handling dismissals below */
              ...
              swalWithBootstrapButtons.fire({
                  title: "Cancelado!",
                  text: "Cancelo la eliminación de marca!!",
                  icon: "error"
              });
          }
      });
          ...
          if (result.isConfirmed) {
              activar(id).then(respuesta=>{
                  if(respuesta.mensaje == 'success'){
                      swalWithBootstrapButtons.fire({
                          title: "Activado!",
                          text: "El marca fue activado "+nombre+".",
                          icon: "success"
                      });
                      obtener_marcas();
                  }else if(respuesta.mensaje == 'error_sesion'){
            ...
            /* Read more about handling dismissals below */
              ...
              swalWithBootstrapButtons.fire({
                  title: "Cancelado!",
                  text: "Cancelo la activación de marca.",
                  icon: "error"
              });
          }
      });
