$(document).ready(function(){
  ...
  async function backup(){
      let funcion = "backup";
      let data = await fetch('../controllers/backupController.php',{
          method: "POST",
          headers: {'Content-Type': 'application/x-www-form-urlencoded'},
          body: 'funcion='+funcion
      ...
      if(data.ok){
          ...
          try {
              console.log(response);
              if(response == ""){
                Cerrar_Loader('exito_backup');
              }else{
                Cerrar_Loader('error_backup');
              }
          } catch (error) {
          console.error(error);
          console.log(response);
  ...
  $(document).on('click','#boton_backup',(e)=>{
    Mostrar_Loader('GenerarBackup');
    backup();
    e.preventDefault();
  });
  ...
  function Mostrar_Loader(Mensaje){
    ...
    switch (Mensaje) {
      case 'GenerarBackup':
        texto = 'Se esta generando el copia de seguridad, porfavor espere...';
        mostrar = true;
  ...
  function Cerrar_Loader(Mensaje){
    ...
    switch (Mensaje) {
      case 'exito_backup':
        tipo = 'success';
        texto = 'La copia de seguridad fue generado correctamente.';
        mostrar = true;
        break;
        case 'error_backup':
          tipo = 'error';
          texto = 'La copia de seguridad no pudo generarse, comuniquese con el personal de sistemas.';
          mostrar = true;
          break;
