$(document).ready(function(){
                      ...
                      <div class="card bg-light d-flex flex-fill">
                          ...
                          <div class="card-body pt-0">
                              <div class="row">
                                  ...
                                  <li class="mediun"><span class="fa-li"><i class="fas fa-lg fa-angle-double-right"></i></span> RUC: ${datos.ruc}</li>
                                  ...
                                  <img src="../util/img/proveedor/${datos.avatar}" alt="tinta" width="80" height="90" class="rounded-circle img-fluid">
                                  </div>
                              </div>
                          </div>
                          ...
                          <div class="card-footer">
                              <div class="text-right">`
                                  if(datos.estado =='A'){
                                    ...
                                    correo="${datos.correo}" 
                                    class="editar_proveedor btn btn-circle btn-sm btn-outline-success ml-2 mb-2 mr-2" title="Editar dato" data-toggle="modal" data-target="#editar_proveedor"><i class="fas fa-pen-alt"></i></button>
                                    <button id="${datos.id}"
                                    nombre="${datos.nombre}"
                                    avatar="${datos.avatar}"
                                    class="editar_avatar btn btn-circle btn-sm btn-outline-info ml-2 mr-2 mb-2" title="editar avatar" data-toggle="modal" data-target="#editar_avatar"><i class="far fa-image"></i></button>
                                    <button id="${datos.id}"
                                    docum="${datos.documento}"
                                    ruc="${datos.ruc}"
                                    avatar="${datos.avatar}"
                                    class="eliminar btn btn-circle btn-sm btn-outline-danger ml-2 mr-2 mb-2" title="Eliminar dato"><i class="fas fa-trash"></i></button>
                                    `
                                      ...
                                      template+=`
                                          ...
                                          ruc="${datos.ruc}"
                                          avatar="${datos.avatar}"
                                          class="activar btn btn-circle btn-sm btn-outline-dark ml-2 mr-2 mb-2" title="Activar proveedor"><i class="fas fa-plus"></i></button>
  ...
  async function registro_proveedores(datos){
      let data = await fetch('../controllers/ProveedorController.php',{
        method: "POST",
      ...
      if(data.ok){
        ...
        try {
          ...
          if(respuesta.mensaje == 'success'){
            toastr.success('Se registro el proveedor','Exito!')
            obtener_proveedores();
            $('#crear_proveedor').modal('hide');
            $("#form-registrar").trigger('reset');
          }else if(respuesta.mensaje == 'error_provee'){
            Swal.fire({
                icon: 'error',
                title: 'El proveedor ya existe',
                text: 'El proveedor ya existe, pongase en contacto con el administrador del sistema'
            });
            $('#crear_proveedor').modal('hide');
            $("#form-registrar").trigger('reset');
  ...
  $.validator.setDefaults({
    submitHandler: function () {
      ...
      let funcion = "registro_proveedores";
      datos.append('funcion',funcion);
      registro_proveedores(datos);
    }
  });
  ...
  $('#form-registrar').validate({
      ...
      messages: {
        ...
        ruc: {
          ...
          minlength: "*Se permite minimo 11 caracteres",
          maxlength: "*Se permite maximo 11 caracteres"
        },
        nombre: {
          required: "*Dato requerido",
        },
  ...
  async function editar_proveedores(datos){
      let data = await fetch('../controllers/ProveedorController.php',{
        method: "POST",
        //en este caso ya no necesita headers por tema de fordata
        //headers: {'Content-Type': 'application/x-www-form-urlencoded'},
      ...
      if(data.ok){
        ...
        try {
          let respuesta = JSON.parse(response);
          if(respuesta.mensaje == 'success'){
              toastr.success('Se Edito el proveedor','Exito!')
              obtener_proveedores();
  ...
  $.validator.setDefaults({
    submitHandler: function () {
      ...
      let funcion = "editar_proveedores";
      datos.append('funcion',funcion);
      editar_proveedores(datos);
    }
  });
  jQuery.validator.addMethod("letras",function(value) {
      let campo = value.replace(/ /g,"");
  ...
  $(document).on('click','.editar_avatar',(e)=>{
    ...
    $("#avatar").attr('src','../util/img/proveedor/'+avatar);
    $("#avatar1").val(avatar);
  });
  ...
  $(document).on('click','.eliminar',(e)=>{
      ...
      swalWithBootstrapButtons.fire({
          title: 'Deseas eliminar proveedor '+docum+': '+ruc+' ?',
          imageUrl: '../util/img/proveedor/'+avatar,
          imageWidth: 200,
          imageHeight: 200,
          ...
          if (result.isConfirmed) {
              eliminar(id).then(respuesta=>{
                  if(respuesta.mensaje == 'success'){
                      swalWithBootstrapButtons.fire({
                          title: "Eliminado!",
                          text: "El proveedor fue eliminado "+docum+": "+ruc+".",
                          icon: "success"
                      });
                      obtener_proveedores();
                  }else if(respuesta.mensaje == 'error_decrypt'){
            ...
            /* Read more about handling dismissals below */
              ...
              swalWithBootstrapButtons.fire({
                  title: "Cancelado!",
                  text: "Cancelo la eliminación de proveedor",
                  icon: "error"
              });
          }
      });
  ...
  $(document).on('click','.activar',(e)=>{
      ...
      swalWithBootstrapButtons.fire({
          title: 'Desea volver activar a '+docum+': '+ruc+'?',
          imageUrl: '../util/img/proveedor/'+avatar,
          imageWidth: 200,
          imageHeight: 200,
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
              activar(id).then(respuesta=>{
                  if(respuesta.mensaje == 'success'){
                      swalWithBootstrapButtons.fire({
                          title: "Activado!",
                          text: "El proveedor fue activado "+docum+": "+ruc+".",
                          icon: "success"
                      });
                      obtener_proveedores();
                  }else if(respuesta.mensaje == 'error_sesion'){
            ...
            /* Read more about handling dismissals below */
              ...
              swalWithBootstrapButtons.fire({
                  title: "Cancelado!",
                  text: "Cancelo la activación de proveedor.",
                  icon: "error"
              });
          }
      });
