    ...
    if(data.ok){
      ...
      try {
        ...
        for (let i = 0; i <= json.length; i++) {
          $('#notif_contador').html(i);
        }
        //let fecha_moment = moment(historial[0].fecha,'DD/MM/YYYY');
        let template = '';
        json.forEach(notificacion => {
          let fecha_moments = moment(notificacion.fecha,'YYYY/MM/DD HH:mm:ss');
          let hora_moment;
          hora_moment = fecha_moments.format('LLLL');
          let dia = moment().startOf(notificacion.dia).fromNow();   
          template+=`
          <a class="dropdown-item d-flex align-items-center">
              <div class="mr-3">`
                  if(notificacion.estado == 'warning'){
                    template+=`
                    <div class="icon-circle bg-warning">
                        <i class="fas fa-exclamation-triangle text-white"></i>
                    </div>`
                  }
                  if(notificacion.estado == 'danger'){
                    template+=`
                    <div class="icon-circle bg-danger">
                        <i class="fas fa-exclamation-triangle text-white"></i>
                    </div>`
              ...
              template+=`</div>
              <div>
                  <div class="small text-gray-500">${hora_moment} (${dia}) </div>
                  <span>Cliente: ${notificacion.nombre}</span>
                  <hr>`
                  if(notificacion.estado == 'warning'){
                    template+=`
                    <span class="font-weight-bold">Está ${notificacion.estaus} de pago del servicio.!</span>
                    <span class="font-weight-bold">Deuda: ${notificacion.deuda} -- Saldo: ${notificacion.saldo}</span>`
                  }
                  if(notificacion.estado == 'danger'){
                    template+=`
                    <span class="font-weight-bold">Está ${notificacion.estatus} de pago del servicio.!</span>
                    <hr>
                    <span class="font-weight-bold">Deuda: ${notificacion.deuda} -- Saldo: ${notificacion.saldo}</span>`
                  }
              template+=`</div>
