$(document).ready(function(){
    ...
    async function obtener_MediosPagos(){
        ...
        if(data.ok){
          let response = await data.text();
          try {
            let MediosPagos = JSON.parse(response);
            //console.log(usuarios);
            $('#medios_pago').DataTable({
              data: MediosPagos,
              "aaSorting": [],
                        ...
                        <div class="card bg-light d-flex flex-fill">
                            ...
                            <div class="card-body pt-0">
                                <div class="row">
                                    ...
                                    <img src="../util/img/${datos.avatar}" alt="tinta" width="80" height="90" class="rounded-circle img-fluid">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer">
                                <div class="text-right">`
                                    if(datos.estado =='A'){
                                      ...
                                      <button id="${datos.id}"
                                      nombre="${datos.nombre}" 
                                      class="editar_MediosPagos btn btn-circle btn-sm btn-outline-success ml-2 mb-2 mr-2" title="Editar Medios de pago" data-toggle="modal" data-target="#editar_MediosPago"><i class="fas fa-pen-alt"></i></button>
                                      <button id="${datos.id}"
                                      nombre="${datos.nombre}"
                                      avatar="${datos.avatar}"
                                      class="editar_avatar btn btn-circle btn-sm btn-outline-info ml-2 mr-2 mb-2" title="Cambiar Foto" data-toggle="modal" data-target="#editar_avatar"><i class="far fa-image"></i></button>
                                      <button id="${datos.id}"
                                      nombre="${datos.nombre}"
                                      avatar="${datos.avatar}"
                                      ...
                                      `
                                        ...
                                        template+=`
                                            ...
                                            nombre="${datos.nombre}"
                                            avatar="${datos.avatar}"
                                            class="activar btn btn-circle btn-sm btn-outline-dark ml-2 mr-2 mb-2" title="Activar medios pago"><i class="fas fa-plus"></i></button>
    ...
    async function registro_MediosPagos(datos){
        let data = await fetch('../controllers/MediosPagoController.php',{
          method: "POST",
        ...
        if(data.ok){
          ...
          try {
            ...
            if(respuesta.mensaje == 'success'){
              toastr.success('Se registro el Medios de pago','Exito!')
              obtener_MediosPagos();
              $('#Crear_pago').modal('hide');
              $("#form-registrar").trigger('reset');
            }else if(respuesta.mensaje == 'error_pago'){
              Swal.fire({
                  icon: 'error',
                  title: 'El medios de pago ya existe',
                  text: 'El medios de pago ya existe, pongase en contacto con el administrador del sistema'
              });
              $('#Crear_pago').modal('hide');
              $("#form-registrar").trigger('reset');
    ...
    $.validator.setDefaults({
        submitHandler: function () {
          ...
          let funcion = "registro_MediosPagos";
          datos.append('funcion',funcion);
          registro_MediosPagos(datos);
        }
    });
    ...
    $(document).on('click','.editar_MediosPagos',(e)=>{
    ...
    async function editar_MediosPago(datos){
        let data = await fetch('../controllers/MediosPagoController.php',{
          method: "POST",
          //en este caso ya no necesita headers por tema de fordata
          //headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        ...
        if(data.ok){
          ...
          try {
            ...
            console.log(respuesta);
            if(respuesta.mensaje == 'success'){
                toastr.success('Se Edito el medios de pago','Exito!')
                obtener_MediosPagos();
                $('#editar_MediosPago').modal('hide');
    ...
    $.validator.setDefaults({
      submitHandler: function () {
        ...
        let funcion = "editar_MediosPagos";
        datos.append('funcion',funcion);
        editar_MediosPago(datos);
      }
    });
    jQuery.validator.addMethod("letras",function(value) {
        let campo = value.replace(/ /g,"");
    ...
    $(document).on('click','.editar_avatar',(e)=>{
      ...
      $("#avatar").attr('src','../util/img/'+avatar);
      $("#avatar1").val(avatar);
    });
