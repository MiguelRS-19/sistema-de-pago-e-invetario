$(document).ready(function() {
    ...
    async function reporte_mensual(datos){
        let funcion = "reporte_mensual";
        let data = await fetch('../controllers/ingresoMensualController.php',{
            method: "POST",
            body: datos
    ...
    $.validator.setDefaults({
        submitHandler: function () {
            ...
            let datos = new FormData($('#form_mensual')[0]);
            let funcion = "reporte_mensual";
            datos.append('funcion',funcion);
            reporte_mensual(datos);
        }
    });
    $('#form_mensual').validate({
        rules: {
            mes: {
                required: true,
            },
